...
import { Plus, Settings, FolderOpen, Undo, Redo, X, Menu, ChevronLeft, ChevronRight, Users, Mail, Clock, MoreHorizontal, FileText } from "lucide-react"
...
export function DebateFlow() {
  ...
  const handleShareSpeech = async () => {
    ...
    if (!round) {
      alert("This flow is not associated with an active round. Cannot share.")
      return
    }
    ...
    if (emails.length === 0) {
      alert("No participants with emails found in this round.")
      return
    }

    if (confirm(`Share "${selectedSpeech}" with ${emails.length} participants?`)) {
      try {
        const result = await shareSpeech(emails, selectedSpeech, speechContent)
        if (result.success) {
  ...
  const sortedFlows = [...flows].filter(f => !f.archived).sort((a, b) => {
                        ...
                        <div className="bg-[var(--background)] h-full rounded-[var(--border-radius)] flex flex-col">
                          <div className="flex items-center justify-between p-4 border-b border-border">
                            ...
                            <div className="flex items-center gap-1">
                              <Button variant="ghost" size="icon" onClick={handleShareSpeech} title="Share with round participants">
